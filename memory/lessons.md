...
[DogGPT] 2026-04-10 外部测试、脚本、API 结果必须只引用真实返回值；如果实际结果没取到，必须明确说未取到，严禁自创人格类型、枚举值、字段或结论冒充官方输出。
[DogGPT] 2026-04-15 File write UX: 用户明确要求以后不要再出现类似 `Edit: in ~/reddog-scraper/... failed` 这类提示；改文件时优先用整文件写入或其他更稳的写法，避免暴露 edit 失败消息。
